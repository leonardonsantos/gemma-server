...
| `No rule to make target '…/cc/options/liblitert_cc_options.a'` (~100%, final link) | LiteRT-LM's `litert_target_map.cmake` links every listed static lib into `litert_lm_main`, including a recently-added `litert::cc_options` entry. At the pinned LiteRT commit, `cc/options` is an INTERFACE compatibility target whose sources (`litert_compiler_options.cc`) are compiled straight into `liblitert_cc_api.a`, so the standalone `liblitert_cc_options.a` is never produced. The installer drops the stale map entry; the symbols still come from `liblitert_cc_api.a` (also in the map), and nothing references `litert::cc_options` by name |
