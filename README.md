...
| `No rule to make target '…/session_basic.cc'` (or `engine_impl.cc`, `session_factory.cc`, `gemma_model_constraint_provider.cc`) (~97%) | LiteRT-LM's CMake files lag a Bazel refactor and reference renamed/removed sources. The installer realigns `runtime/core/CMakeLists.txt` (`session_basic.cc`→`session_advanced.cc`, `engine_impl.cc`→`engine_advanced_impl.cc`, and the dropped `session_factory` target → an INTERFACE forwarder), and switches the `gemma_model_constraint_provider` target to **import the prebuilt** `libGemmaModelConstraintProvider.so` (a closed-source git-LFS blob the Gemma data processors call) instead of compiling a non-existent `.cc`. The installer fetches the real `.so` (the clone leaves a git-LFS pointer), installs it to `~/.gemma-server/lib`, and the launcher adds that dir to `LD_LIBRARY_PATH` |
