...
| `no member named 'SetKernelBatchSize'` / `'SetDisableDelegateClustering'` / `'SetWeightCacheFd'` (~88–94%) | LiteRT-LM `main` has drifted ahead of the LiteRT commit it pins and calls a few option setters that don't exist there yet (a full scan of the tree against the pinned headers found exactly these three). All are optional tuning hints, so the installer drops the calls from `runtime/executor/{llm_executor_settings_utils,litert_compiled_model_executor_utils}.cc`; default CPU/GPU inference is unaffected |
| `no member named 'SetPerformanceMode'` / `'PerformanceMode'` in `GoogleTensorOptions`, or `'HasQuantization'` / `'PerTensorQuantization'` in `SimpleTensor` (~95%) | More pin drift, this time in the NPU (Google Tensor) and audio executors. These paths only run on the NPU backend, which Gemma-on-Termux never uses (CPU/GPU only). The installer defines LiteRT-LM's own `LITERT_DISABLE_NPU` macro (which `#if`-guards every NPU/GoogleTensor reference in the factory/audio/vision/util sources) and wraps the always-compiled NPU executor TU in the same guard, so the whole NPU path compiles out cleanly |
| `ld.lld: undefined symbol: litert::lm::…` at the final `litert_lm_main` link (`Gemma4DataProcessor::Create`, `FastVlmDataProcessor::Create`, `GetOpenChannelName`, `IsPixelTensorDevice`, `GetAspectRatioPreservingSize`, …) | More Bazel-vs-CMake drift: several sources exist in the tree and are built by Bazel but are absent from any CMake target's source list, so they're never compiled. The super-build auto-links **every** `add_litertlm_library` STATIC archive into `litert_lm_main`, so the installer compiles each orphaned source (`cpu_affinity_utils.cc`, `channel_util.cc`, `image_preprocessor_utils.cc`, `gemma4_data_processor.cc`, `fastvlm_data_processor.cc`) into its own STATIC target and the symbols are pulled in |
| `ld.lld: undefined symbol: LiteRtLmGemmaModelConstraintProvider_*` at the final link | The prebuilt provider `.so` is imported as a CMake target and wired into the data processors, but `litert_lm_main` links the *flattened list of archive paths* in the local aggregate, which doesn't follow that target's transitive interface — so the `.so` never reaches the link line. The installer adds the `.so` path directly to `litert_lm_main`'s link (inside the `--start-group`/`--end-group`) |
